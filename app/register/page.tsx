...
import { sendOtp, verifyOtp, signupUser } from "@/lib/actions/auth";
        ...
        <CardContent className="space-y-4">
          ...
          {!otpState?.success && (
            <form action={sendOtpAction}>
              ...
              {otpState?.message && (
                <p className="text-sm text-center mt-2">{otpState.message}</p>
              )}
              {/* Show OTP for development (if email fails) */}
              {otpState?.devOtp && (
                <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded text-center">
                  <p className="text-sm text-yellow-800">
                    <strong>Development OTP:</strong> {otpState.devOtp}
                  </p>
